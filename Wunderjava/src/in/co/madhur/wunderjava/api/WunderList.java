...
public class WunderList {
	...
	public List<WTask> GetTasks() throws AuthException, NetworkException {
		...
		try {
			...
			if (e.getResponse().getStatus() == 401) {
				...
				throw new AuthException(APIConsts.UNKNWOWN_RETROFIT_ERROR,
						AuthError.UNKNOWN);
		}
	...
	public WTask CreateTask(String listId, String title, String starred,
			String due_date) throws NetworkException, AuthException {
		WTask task = null;

		try {
			task = service.CreateWunderTask(token, listId, title, starred,
					due_date);
		} catch (RetrofitError e) {
	...
	public void DeleteTask(String taskId) throws NetworkException,
			AuthException {
		try {
			service.DeleteList(token, taskId);
		} catch (RetrofitError e) {
